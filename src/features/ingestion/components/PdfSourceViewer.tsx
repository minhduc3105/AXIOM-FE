...
export default function PdfSourceViewer({
  ...
  useLayoutEffect(() => {
    ...
    if (!element) return;
    const updateWidth = () => setContainerWidth(Math.max(280, element.clientWidth - 32));
    updateWidth();
  ...
  useEffect(() => {
    if (pageCount > 0 && pageIndex >= pageCount) onPageIndexChange(pageCount - 1);
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col bg-[#25241f]" ref={widthRef}>
      <div className="flex min-h-12 flex-wrap items-center justify-between gap-2 border-b border-white/10 bg-[#191915] px-3 py-2 text-white">
        ...
        <div className="flex items-center gap-1">
          <Button
            ...
            onClick={() => onZoomChange(Math.max(0.65, Number((zoom - 0.15).toFixed(2))))}
          >
            <ZoomOutIcon />
          </Button>
          ...
          <Button
            ...
            onClick={() => onZoomChange(Math.min(1.6, Number((zoom + 0.15).toFixed(2))))}
          >
            <ZoomInIcon />
          </Button>
          <Toggle
      ...
      <div className="min-h-[520px] flex-1 overflow-auto p-4">
        {loadError ? (
          <div className="grid min-h-[480px] place-items-center rounded-xl border border-red-400/40 bg-red-950/20 p-6 text-center text-sm text-red-100">
            {loadError}
          </div>
        ) : (
          <Document
            file={url}
            loading={<div className="grid min-h-[480px] place-items-center text-sm text-white/70">Loading PDF…</div>}
            onLoadSuccess={(pdf) => {
          ...
          >
            <div className="relative mx-auto w-fit overflow-hidden bg-white shadow-2xl" aria-label={`PDF page for ${fileName}`}>
              <Page
                pageNumber={pageIndex + 1}
                width={renderedWidth}
                renderAnnotationLayer={false}
                renderTextLayer={false}
                loading={<div className="grid min-h-[480px] place-items-center text-sm text-black/60">Rendering page…</div>}
              />
              <SourceBlockOverlay
                blocks={pageBlocks}
                allBlocks={blocks}
