  ...
  return <div className="space-y-6">
    ...
    <div className="grid gap-6 xl:grid-cols-[340px_minmax(0,1fr)]">
      ...
      <Card className="rounded-[32px] border border-[#d8d0c2] bg-[#fffdf8]/90 p-6 dark:border-[#38372f] dark:bg-[#1a1a17]/90">
        ...
        <div className="mt-5 overflow-x-auto rounded-2xl border border-[#d8d0c2] dark:border-[#38372f]"><Table><TableHeader><TableRow><TableHead>Source</TableHead><TableHead>Units</TableHead><TableHead>Key fields</TableHead><TableHead>Risk</TableHead><TableHead>Next action</TableHead></TableRow></TableHeader><TableBody>{rows.map((row, rowIndex) => <TableRow className={rowIndex === selectedIndex ? 'bg-[#eef2ff] dark:bg-[#202844]' : ''} key={row[0]} onClick={() => setSelectedIndex(rowIndex)}>{row.map((cell, cellIndex) => <TableCell key={cell}>{cellIndex === 0 ? <Button variant="ghost" className="justify-start px-0 text-[#2456e8]" type="button" onClick={() => setSelectedIndex(rowIndex)}>{cell}</Button> : cell}</TableCell>)}</TableRow>)}</TableBody></Table></div>
