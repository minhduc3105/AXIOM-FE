  ...
  return <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_420px]">
    <Card className="rounded-[32px] border border-border bg-card p-6">
      ...
      {semanticConcepts.map((concept, index) => <Button
        className={cn('mt-3 min-h-20 w-full flex-col items-start rounded-2xl border border-border bg-card p-4 text-left', index === selectedConcept && 'border-primary bg-primary/10')}
        key={concept.title}
        type="button"
        onClick={() => setSelectedConcept(index)}
        aria-pressed={index === selectedConcept}
        aria-label={`Select semantic concept ${concept.title}`}
      ><strong>{concept.title}</strong><span className="text-sm text-muted-foreground">{concept.copy}</span></Button>)}
      <div className="mt-5 rounded-3xl bg-secondary p-5" aria-live="polite"><strong>Suggested relationship graph · {activeConcept.title}</strong><span className="mt-2 block text-sm leading-relaxed text-muted-foreground">{activeConcept.relationship}<br />{activeConcept.evidence}</span></div>
    </Card>
    <Card className="rounded-[32px] border border-border bg-card p-6"><div className="mb-5 flex items-center justify-between gap-4"><h2 className="text-2xl font-semibold">Review semantic hints</h2>{revisionCount > 0 && <Badge className="bg-status-success/10 text-status-success">Revision {revisionCount} applied</Badge>}</div>{[['High', 'Governed identifiers', 'Detected email and customer_id as governed fields across structured sources.'], ['High', 'Chunk strategy', 'Table rows stay row-addressable; documents split by paragraph/page with source ordering.'], ['Med', 'Search filters', 'segment, region, status, timestamp, currency, renewal_date become filters.'], ['Med', 'Reviewer note', 'Answers involving personal data require approval before external sharing.']].map(([level, title, copy]) => <div className="mb-3 flex gap-3 rounded-2xl bg-secondary p-4" key={title}><Badge className={level === 'High' ? 'h-fit bg-status-warning/10 text-status-warning' : 'h-fit bg-accent text-muted-foreground'}>{level}</Badge><div><strong>{title}</strong><p className="text-sm leading-relaxed text-muted-foreground">{copy}</p></div></div>)}<div className="mt-5 rounded-3xl border border-border bg-secondary p-5"><strong>{busy ? 'Refreshing semantic hints…' : 'Ready to build index?'}</strong><p className="mt-2 text-sm text-muted-foreground">Semantic hints are linked to source files, fields, rows, and document chunks.</p><div className="mt-5 flex flex-wrap gap-3"><Button type="button" disabled={busy} onClick={onApprove}>Approve meaning</Button><Button variant="outline" type="button" disabled={busy} onClick={onRevision}>{busy ? 'Revising…' : 'Request revision'}</Button><Button variant="ghost" type="button" disabled={busy} onClick={onBack}>Back to profile</Button></div></div></Card>
  </div>
}
