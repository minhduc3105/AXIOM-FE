...
export function IngestionJobStatus({
  ...
  return <Card className="rounded-[32px] border border-border bg-card p-6">
    <div className="flex items-start justify-between gap-4">
      <h2 className="text-2xl font-semibold">Import status</h2>
      <Badge className={cn(
        'rounded-full border px-3 py-1',
        successful && 'border-status-success/25 bg-status-success/10 text-status-success',
        failed && 'border-destructive/25 bg-destructive/10 text-destructive',
        !successful && !failed && 'border-border bg-secondary text-muted-foreground',
      )}>{visibleStatus}</Badge>
    </div>

    <div className="mt-5 flex gap-4 rounded-3xl border border-border bg-secondary p-4">
      <span className="grid size-12 shrink-0 place-items-center rounded-2xl bg-primary text-xs font-bold text-primary-foreground">{mark}</span>
      ...
      {transferSteps.map((step, index) => {
        ...
        return <li
          className={cn(
            ...
            complete && 'border-status-success/25 bg-status-success/10 text-status-success',
            current && 'border-info/25 bg-info/10 font-semibold text-info',
            stepFailed && 'border-destructive/25 bg-destructive/10 text-destructive',
          )}
        ...
        >
          <span className={cn(
            ...
            complete && 'border-status-success bg-status-success text-primary-foreground',
            current && 'border-primary bg-primary text-primary-foreground',
            stepFailed && 'border-destructive bg-destructive text-primary-foreground',
    ...
    {active && <div className="mt-4 flex items-center gap-3 rounded-2xl border border-info/25 bg-info/10 p-3 text-sm text-info" role="status">
    ...
    {!job && status === 'idle' && <ul className="mt-5 grid gap-2">
      {idleNotes.map((note) => <li className="flex items-start gap-2 rounded-2xl bg-secondary px-3 py-2 text-sm" key={note}>
    ...
    {job && <div className="mt-5 grid gap-3" role="status" aria-live="polite">
      <div className="rounded-2xl bg-secondary p-4">
        <span className="text-xs font-semibold uppercase tracking-[0.14em] text-muted-foreground">Job ID</span>
        <strong className="mt-1 block break-all font-mono text-sm">{job.job_id}</strong>
      </div>
      <div className="grid grid-cols-2 gap-3">
        <div className="rounded-2xl bg-secondary p-4"><strong className="block text-2xl">{job.records_pulled}</strong><span className="text-sm text-muted-foreground">Records pulled</span></div>
        <div className="rounded-2xl bg-secondary p-4"><strong className="block text-2xl">{job.objects_written}</strong><span className="text-sm text-muted-foreground">Objects stored</span></div>
      </div>
      <dl className="grid gap-2 rounded-2xl border border-border p-4 text-sm">
        <div className="flex justify-between gap-4"><dt className="text-muted-foreground">Started</dt><dd className="text-right">{formatTimestamp(job.started_at)}</dd></div>
        <div className="flex justify-between gap-4"><dt className="text-muted-foreground">Finished</dt><dd className="text-right">{formatTimestamp(job.finished_at)}</dd></div>
      </dl>
    </div>}

    {(error || job?.error_message) && <div className="mt-5 rounded-2xl border border-destructive/25 bg-destructive/10 p-4 text-sm text-destructive" role="alert">
      {job?.error_message || error}
    </div>}

    {successful && <div className="mt-5 rounded-2xl border border-status-success/25 bg-status-success/10 p-4 text-sm text-status-success">
