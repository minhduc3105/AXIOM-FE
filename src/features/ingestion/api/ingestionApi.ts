...
function isJobFilesPage(value: unknown): value is JobFilesPage {
  ...
  if (!isPositiveInteger(pagination.page)
    ...
    || pagination.total_pages !== Math.ceil(pagination.total_count / pagination.page_size)
    || value.count > pagination.page_size) return false
...
export async function getFilesForJob(
  ...
  const normalizedJobId = jobId.trim()
  if (!normalizedJobId) throw new Error('An ingestion job ID is required.')
  if (!isPositiveInteger(page)) throw new Error('The job files page must be a positive integer.')
  ...
  if (!response.ok) throw new Error(await getHttpError(response, 'Job file discovery'))
  if (response.status !== 200) {
    throw new Error(`Job file discovery returned unexpected HTTP ${response.status}.`)
  }

  const payload: unknown = await response.json()
  if (!isJobFilesPage(payload)
...
export async function getAllFilesForJob(
  ...
  if (keys.size !== files.length) throw new Error('The job files response contained duplicate object keys.')

  for (let page = 2; page <= firstPage.pagination.total_pages; page += 1) {
    const nextPage = await getFilesForJob(jobId, page, 100, signal)
    if (nextPage.organization_id !== firstPage.organization_id
    ...
    for (const file of nextPage.files) {
      if (keys.has(file.key)) throw new Error('The job files response contained duplicate object keys.')
      keys.add(file.key)
      files.push(file)
    }
  }
