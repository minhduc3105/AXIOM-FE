...
const pageTitles = {
  ...
  processing: 'Track document processing in AXIOM Corpus',
...
export function IngestionPage({ onBack }: IngestionPageProps) {
        ...
        : workflow.stage === 's3' || workflow.stage === 'snowflake'
          ? workflow.connectorJobStatus === 'completed'
            ...
            : workflow.connectorJobStatus === 'polling'
              ...
              : workflow.connectorJobStatus === 'failed' || workflow.connectorJobStatus === 'status_error'
  ...
  >
      ...
      {workflow.stage === 's3' && <S3Form connection={workflow.s3Connection} status={workflow.connectorJobStatus} job={workflow.ingestionJob} error={workflow.connectorError} onChange={workflow.updateS3Connection} onSubmit={() => void workflow.submitS3Import()} onRetryStatus={() => void workflow.retryIngestionStatus()} onNewImport={workflow.startNewConnectorImport} onBack={workflow.openCatalog} />}
      {workflow.stage === 'snowflake' && <SnowflakeForm connection={workflow.snowflakeConnection} status={workflow.connectorJobStatus} job={workflow.ingestionJob} error={workflow.connectorError} onChange={workflow.updateSnowflakeConnection} onSubmit={() => void workflow.submitSnowflakeImport()} onRetryStatus={() => void workflow.retryIngestionStatus()} onNewImport={workflow.startNewConnectorImport} onBack={workflow.openCatalog} />}
      {workflow.stage === 'upload' && <UploadWorkspace files={workflow.files} selectedFileId={workflow.selectedFileId} uploadStatus={workflow.uploadStatus} uploadResult={workflow.uploadResult} onFiles={workflow.addFiles} onSelectFile={workflow.selectFile} onUpload={() => void workflow.uploadSelectedFiles()} onProcessing={() => workflow.navigateProgress('pipeline')} onBack={workflow.openSource} />}
      {workflow.stage === 'processing' && workflow.uploadResult && <DocumentProcessingWorkspace uploadResult={workflow.uploadResult} status={workflow.documentProcessingStatus} results={workflow.documentProcessingResults} error={workflow.documentProcessingError} onRetry={() => void workflow.retryDocumentProcessingStatus()} onBack={() => workflow.navigateProgress('transfer')} />}
