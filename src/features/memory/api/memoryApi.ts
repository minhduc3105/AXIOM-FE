...
const intentBase = (
  import.meta.env.VITE_MEMORY_API_BASE_URL ?? "/api/v1"
).replace(/\/$/, "");
...
export const memoryScope: MemoryScope = {
  tenant_id:
    import.meta.env.VITE_MEMORY_TENANT_ID ??
...
async function authenticatedRequest<T>(
  ...
  const response = await authFetch(intelligenceApiUrl(`/api/v1${path}`), {
    ...options,
    headers: {
...
function normalizeMemorySnapshot(value: unknown): MemorySnapshot {
  const raw = asRecord(value);
  return {
    user_markdown: stringValue(raw.user_markdown),
    memory_markdown: stringValue(raw.memory_markdown),
  };
}
...
function normalizeMemory(
  ...
  return {
    ...
    backend: reme
      ? "reme"
      : stringValue(raw.backend) === "postgresql"
        ? "postgresql"
        : "unknown",
    score: score ?? numberValue(raw.score),
    matched_by: arrayValue(raw.matched_by).filter(
...
export async function getServiceStatus(): Promise<{
  ...
  return {
    intent: intent.status === "fulfilled" ? "online" : "offline",
    reme: reme.status === "fulfilled" ? "online" : "offline",
  };
}

export async function getMemorySnapshot(): Promise<MemorySnapshot> {
  return normalizeMemorySnapshot(
    await authenticatedRequest<unknown>("/internal-memory"),
  );
}
...
export async function listMemories(
  ...
  const data = asRecord(
    await request<unknown>(intentBase, `/memories?${scopeParams(scope)}`),
  );
...
export async function getMemory(memoryId: string, scope = memoryScope) {
  return normalizeMemory(
    await request<unknown>(
      intentBase,
      `/memories/${encodeURIComponent(memoryId)}?tenant_id=${encodeURIComponent(scope.tenant_id)}`,
    ),
  );
}
...
export async function searchMemories(
  query: string,
  options: { type?: string; searchType?: string } = {},
  scope = memoryScope,
) {
  const data = asRecord(
    await request<unknown>(intentBase, "/memories/search", {
      method: "POST",
      body: JSON.stringify({
        ...scope,
        query,
        memory_types: options.type ? [options.type] : undefined,
...
export async function deleteMemory(memoryId: string, scope = memoryScope) {
  const params = new URLSearchParams({
    tenant_id: scope.tenant_id,
    actor_id: scope.user_id ?? "axiom-user",
    reason: "Deleted from Memory Management",
  });
  await request(
    intentBase,
    `/memories/${encodeURIComponent(memoryId)}?${params}`,
    { method: "DELETE" },
  );
}
...
export async function feedbackMemory(
  memoryId: string,
  observed_result: FeedbackResult,
  scope = memoryScope,
) {
  return request(intentBase, "/memory-feedback", {
    method: "POST",
    body: JSON.stringify({
      tenant_id: scope.tenant_id,
      memory_id: memoryId,
      ...
      trace_id: `memory-feedback-${Date.now()}`,
    }),
  });
}
...
export async function recallMemory(
  ...
  const raw = asRecord(
    await request<unknown>(intentBase, "/personalization/recall", {
      method: "POST",
      body: JSON.stringify({
        ...scope,
        message: query,
  ...
  return {
    memories: arrayValue(raw.memories).map((item) => normalizeMemory(item)),
    bounded_context: stringValue(raw.bounded_context),
    raw,
  };
}

export async function captureFromIntent(
  storedIntentId: string,
  scope = memoryScope,
) {
  return request(intentBase, "/personalization/capture", {
    method: "POST",
    body: JSON.stringify({
      ...scope,
      stored_intent_id: storedIntentId,
      trace_id: `memory-capture-${Date.now()}`,
    }),
  });
}
...
export async function queueExtraction(
  ...
  const raw = asRecord(
    await request<unknown>(intentBase, "/memory-captures", {
      method: "POST",
      body: JSON.stringify({
        ...scope,
        source_intent_ids: [storedIntentId],
        idempotency_key: `memory-pg-capture:${storedIntentId}:${Date.now()}`,
      }),
    }),
  );
...
export async function getExtractionJob(
  ...
  const raw = asRecord(
    await request<unknown>(
      intentBase,
      `/memory-extraction-jobs/${encodeURIComponent(id)}?tenant_id=${encodeURIComponent(scope.tenant_id)}`,
    ),
  );
  return {
    id,
    status: ["queued", "captured", "failed"].includes(stringValue(raw.status))
      ? (stringValue(raw.status) as ExtractionJob["status"])
      : "unknown",
    message: stringValue(raw.detail) || stringValue(raw.message) || null,
    raw,
  };
}

export async function forgetReme(memory: MemoryRecord, scope = memoryScope) {
  const source = memory.source_refs.find((ref) =>
    ref.source_id.startsWith("reme:"),
  );
  if (!source) throw new Error("This memory is not backed by ReMe.");
  return request(intentBase, "/memories/forget", {
    method: "POST",
    body: JSON.stringify({
      ...scope,
      ...
      actor_id: scope.user_id ?? "axiom-user",
      reason: "Forgotten from Memory Management",
    }),
  });
}
...
export async function listExperiences(scope = memoryScope) {
  const raw = asRecord(
    await request<unknown>(intentBase, `/experiences?${scopeParams(scope)}`),
  );
...
export async function getExperience(id: string, scope = memoryScope) {
  return normalizeExperience(
    await request<unknown>(
      intentBase,
      `/experiences/${encodeURIComponent(id)}?tenant_id=${encodeURIComponent(scope.tenant_id)}`,
    ),
  );
}
export async function searchExperiences(query: string, scope = memoryScope) {
  const raw = asRecord(
    await request<unknown>(intentBase, "/experiences/search", {
      method: "POST",
      body: JSON.stringify({ ...scope, query, limit: 50 }),
    }),
  );
  return arrayValue(raw.experiences).map(normalizeExperience);
}
export async function createExperience(
  ...
  return normalizeExperience(
    await request<unknown>(intentBase, "/experiences", {
      method: "POST",
      body: JSON.stringify({
        ...scope,
        goal,
        situation: { source: "memory-management" },
        actions: [{ order: 1, action: "captured_from_ui" }],
        outcome: { status: "success", user_confirmed: true },
        lesson,
        confidence: 0.8,
        source_refs: [{ source_type: "artifact", source_id: sourceId }],
      }),
    }),
  );
}
export async function listProcedures(scope = memoryScope) {
  const raw = asRecord(
    await request<unknown>(intentBase, `/procedures?${scopeParams(scope)}`),
  );
...
export async function getProcedure(id: string, scope = memoryScope) {
  return normalizeProcedure(
    await request<unknown>(
      intentBase,
      `/procedures/${encodeURIComponent(id)}?tenant_id=${encodeURIComponent(scope.tenant_id)}`,
    ),
  );
}
export async function searchProcedures(query: string, scope = memoryScope) {
  const raw = asRecord(
    await request<unknown>(intentBase, "/procedures/search", {
      method: "POST",
      body: JSON.stringify({ ...scope, query, limit: 50 }),
    }),
  );
...
export async function createProcedure(
  ...
  return normalizeProcedure(
    await request<unknown>(intentBase, "/procedures", {
      method: "POST",
      body: JSON.stringify({
        ...scope,
        name,
        trigger,
        preconditions: [],
        steps: [{ order: 1, action: "review_memory" }],
        failure_modes: [],
        confidence: 0.8,
        source_refs: [{ source_type: "artifact", source_id: sourceId }],
      }),
...
export async function feedbackProcedure(
  id: string,
  observed_result: Exclude<FeedbackResult, "irrelevant">,
  scope = memoryScope,
) {
  return request(intentBase, `/procedures/${encodeURIComponent(id)}/feedback`, {
    method: "POST",
    body: JSON.stringify({
      tenant_id: scope.tenant_id,
      observed_result,
      actor_id: scope.user_id,
      trace_id: `procedure-feedback-${Date.now()}`,
    }),
  });
}

export async function remeOperation(
  ...
  const actions = {
    capture: [
      "/auto_memory",
      {
        session_id: `${memoryScope.tenant_id}-${memoryScope.user_id ?? "user"}`,
        messages: [
          {
            role: "user",
            name: memoryScope.user_id ?? "user",
            content: input,
