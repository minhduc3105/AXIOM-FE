    ...
    <>
      ...
      <div className="grid gap-2 rounded-xl border border-border/70 bg-secondary/45 p-3">
        <p className="text-sm font-medium">
          {report.dashboard?.headline.title ||
            report.title ||
            "Report generation"}
        </p>
        <p className="text-sm leading-5 text-muted-foreground">
          {report.dashboard?.headline.summary ||
            report.summary ||
            "No narrative summary was returned."}
        </p>
      </div>
      <section className="grid gap-2" aria-labelledby="report-detail-sources">
        <h3 id="report-detail-sources" className="text-sm font-semibold">
          Source files
        </h3>
          ...
          {report.sources.length ? (
            report.sources.map((source) => (
              <li
                ...
                className="flex min-w-0 items-center justify-between gap-3 rounded-lg border border-border/70 px-3 py-2 text-sm"
              >
      ...
      {report.dashboard?.metrics.length ? (
        <>
          <Separator />
          <section
            className="grid gap-2"
            aria-labelledby="report-detail-metrics"
          >
            <h3 id="report-detail-metrics" className="text-sm font-semibold">
              Extracted metrics
            </h3>
            <ul className="grid gap-2">
              {report.dashboard.metrics.map((metric) => (
                <li
                  key={metric.id}
      ...
      <Button
        variant="outline"
        className="w-full"
        disabled={!report.report_available}
