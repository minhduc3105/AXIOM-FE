    ...
    <div className="min-w-0">
      <div className="flex flex-col gap-3 border-b px-5 py-4 lg:flex-row lg:items-center">
        <label className="relative min-w-0 flex-1 lg:max-w-md">
          ...
          <Input value={search} onChange={(event) => onSearchChange(event.target.value)} placeholder="Search files" className="pl-9" />
        </label>
      ...
      {loading && !result ? <TableSkeleton /> : totalUnfilteredCount === 0 ? (
        <DataEmptyState
        ...
        <DataEmptyState title="No matching files" description="Adjust the search to see more results." />
      ) : (
        <>
              ...
              <TableHeader className="bg-muted/45">
                <TableRow className="hover:bg-transparent">
                  {([
                    ...
                    <TableHead key={label} aria-sort={field !== null && sortBy === field ? (sortOrder === "asc" ? "ascending" : "descending") : "none"} className={`h-11 px-4 text-xs font-semibold ${className}`}>
                      {field ? (
                        <button type="button" onClick={() => onSortChange(field)} className={`inline-flex items-center gap-1.5 rounded-md py-1 outline-none focus-visible:ring-3 focus-visible:ring-primary/25 ${field === "size" ? "float-right" : ""}`}>
                          {label}{sortIcon(field)}
                        </button>
                      ) : label}
                    </TableHead>
                  ))}
                  <TableHead className="h-11 w-24 px-4"><span className="sr-only">Actions</span></TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                {files.map((file) => (
                  <TableRow key={file.key} className="group">
                    <TableCell className="max-w-0 px-4 py-3.5">
                      <div className="flex min-w-0 items-center gap-3">
                        <span className="flex size-10 shrink-0 items-center justify-center rounded-xl border bg-muted text-primary"><FileIcon className="size-4" /></span>
                        <div className="min-w-0">
                          {file.canInspect ? (
                            <button type="button" onClick={() => onInspect(file)} className="block max-w-full truncate text-left text-sm font-medium outline-none hover:text-primary focus-visible:rounded focus-visible:ring-3 focus-visible:ring-primary/25">
                              {file.name}
                            </button>
                          ) : <strong className="block truncate text-sm font-medium">{file.name}</strong>}
                          <span className="block truncate text-xs text-muted-foreground" title={file.key}>{file.key}</span>
                        </div>
                      </div>
                    </TableCell>
                    ...
                    <TableCell className="px-4 py-3.5 text-right">
                      <div className="flex justify-end gap-1">
                        <Button variant="ghost" size="icon-sm" onClick={() => onInspect(file)} disabled={!file.canInspect} aria-label={`View processed result for ${file.name}`} title={file.canInspect ? `View processed result for ${file.name}` : "Processed result is not ready"}><FileSearchIcon /></Button>
                        <a href={file.downloadUrl} className="inline-flex size-8 items-center justify-center rounded-md text-muted-foreground outline-none hover:bg-muted focus-visible:ring-3 focus-visible:ring-primary/25" aria-label={`Download ${file.name}`} title={`Download ${file.name}`}><DownloadIcon className="size-4" /></a>
                      </div>
                    </TableCell>
                  </TableRow>
                ))}
          ...
          <div className="flex flex-col gap-3 border-t px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
            <div className="flex items-center gap-2 text-sm text-muted-foreground"><span>Rows per page</span><Select value={String(pageSize)} onValueChange={(value) => onPageSizeChange(Number(value))}><SelectTrigger className="w-20" aria-label="Rows per page"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="10">10</SelectItem><SelectItem value="20">20</SelectItem><SelectItem value="50">50</SelectItem></SelectContent></Select></div>
            <div className="flex items-center justify-between gap-3 sm:justify-end"><span className="text-sm tabular-nums text-muted-foreground">Page {page} of {totalPages}</span><Button variant="outline" size="sm" disabled={page <= 1 || loading} onClick={() => onPageChange(page - 1)}>Previous</Button><Button variant="outline" size="sm" disabled={page >= totalPages || loading} onClick={() => onPageChange(page + 1)}>Next</Button></div>
          </div>
        </>
      )}
    </div>
